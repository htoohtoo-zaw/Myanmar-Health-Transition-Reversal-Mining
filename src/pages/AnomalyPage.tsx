    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-[#E4E9F2] rounded-[8px] p-5 shadow-[0_1px_2px_rgba(11,15,25,0.06)]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
          ...
          <span className="text-[11px] font-mono px-2.5 py-1 rounded bg-[#FDF0EF] text-[#C4453F] border border-[#F4C5C2] font-semibold">
            ...
            <AreaChart data={ANOMALY_SERIES} margin={{ top: 10, right: 20, left: -15, bottom: 0 }}>
              <defs>
                <linearGradient id="anomalyGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#C4453F" stopOpacity={0.4} />
                  <stop offset="95%" stopColor="#1C4BBC" stopOpacity={0.02} />
                </linearGradient>
              </defs>
              ...
              <YAxis domain={[0, 1.0]} stroke="#60636A" fontSize={12} tickLine={false} />
              <Tooltip
                contentStyle={{
              ...
              <ReferenceLine
                ...
                stroke="#C68A1E"
                strokeDasharray="3 3"
                label={{ value: 'Elevated Anomaly Threshold (0.50)', fill: '#C68A1E', fontSize: 11 }}
              />
              <ReferenceLine
                x={2021}
                stroke="#C4453F"
                strokeDasharray="4 4"
                label={{ value: '2021 Peak Fracture', position: 'top', fill: '#C4453F', fontSize: 11, fontWeight: 600 }}
              />
              <Area
                type="monotone"
                dataKey="score"
      ...
      <div className="bg-white border border-[#E4E9F2] rounded-[8px] p-5 shadow-[0_1px_2px_rgba(11,15,25,0.06)] space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between text-[12px] font-semibold text-[#1C4BBC] mb-1">
            <span>A. National Surveillance Completeness Index (0.0 – 1.0)</span>
            <span className="font-mono text-[#C4453F]">Dropped 1.00 → 0.80 in 2021</span>
          </div>
          <div className="h-[140px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={ANOMALY_SERIES} margin={{ top: 5, right: 20, left: -15, bottom: 0 }}>
                ...
                <YAxis domain={[0.7, 1.05]} stroke="#60636A" fontSize={11} tickLine={false} />
                <Tooltip
                  contentStyle={{
                    backgroundColor: '#FFFFFF',
                    borderColor: '#E4E9F2',
                ...
                />
                <ReferenceLine x={2021} stroke="#C4453F" strokeDasharray="3 3" />
                <Line
                  type="monotone"
        ...
        <div className="pt-2 border-t border-[#E4E9F2]">
          <div className="flex items-center justify-between text-[12px] font-semibold text-[#C68A1E] mb-1">
            <span>B. Mean Routine Immunisation Coverage (%)</span>
            <span className="font-mono text-[#C4453F]">Halved: 92% → 45% in 2021</span>
          </div>
          <div className="h-[140px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={ANOMALY_SERIES} margin={{ top: 5, right: 20, left: -15, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#CAD3E6" vertical={false} />
                <XAxis dataKey="year" stroke="#60636A" fontSize={11} tickLine={false} />
                <YAxis domain={[30, 100]} stroke="#60636A" fontSize={11} tickLine={false} unit="%" />
                <Tooltip
                  contentStyle={{
                    backgroundColor: '#FFFFFF',
                    borderColor: '#E4E9F2',
      ...
      <div className="bg-white border border-[#E4E9F2] rounded-[8px] p-5 shadow-[0_1px_2px_rgba(11,15,25,0.06)]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {OUTBREAK_YEARS.map((ob, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-[8px] bg-[#EDF1FA]/60 border border-[#CAD3E6] flex flex-col justify-between"
            >
              <div>
                <span className="text-[13px] font-bold text-[#0B0F19]">
                  {ob.disease}
                </span>
                <div className="text-[11px] text-[#60636A] mt-0.5">
                  Algorithm: {ob.method}
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {ob.years.map((yr) => (
                  <span
                    key={yr}
                    className="px-2 py-0.5 rounded-[4px] font-mono text-[12px] font-bold bg-white text-[#C4453F] border border-[#F4C5C2]"
                  >
                    {yr}
                  </span>
                ))}
