...
export const AnomalyPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-[26px] md:text-[30px] font-bold text-[var(--c-ink)]"> {t('Anomaly Detection & Surveillance Completeness')} </h1>
        <p className="text-[14px] text-[var(--c-muted)] mt-1 max-w-4xl"> {t('Disentangling genuine public health outbreaks from structural surveillance collapse using multidimensional Isolation Forests and reporting completeness audits.')} </p>
      </div>
      ...
      <Callout
        ...
        title={t('Why Simultaneous Drops in Cases and Coverage Prove Data Collapse')}
      > {t('If measles immunisation plummets from 92% to 44%, true community disease transmission')} <em>{t('must increase')}</em>{t('. The fact that officially reported measles case numbers also dropped from thousands to 42 cases in 2021 is an administrative impossibility. It proves that public health facilities and hospital reporting pipelines fractured, creating an illusion of disease absence.')} </Callout>
      ...
      <div className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[8px] p-5 shadow-[var(--c-shadow-sm)]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
          <div>
            <h3 className="text-[16px] font-semibold text-[var(--c-ink)]"> {t('Isolation Forest Temporal Anomaly Score (2000–2024)')} </h3>
            <p className="text-[12px] text-[var(--c-muted)]"> {t('Trained on cross-indicator multidimensional shifts. Peak anomaly scores coincide with 2021–2022 disruption.')} </p>
          </div>
          <span className="text-[11px] font-mono px-2.5 py-1 rounded bg-[var(--c-danger-bg)] text-[var(--c-danger)] border border-[var(--c-danger-border)] font-semibold"> {t('2021 Peak Score: 0.88 / 1.00')} </span>
        </div>
      ...
      <div className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[8px] p-5 shadow-[var(--c-shadow-sm)] space-y-4">
        <div>
          <h3 className="text-[16px] font-semibold text-[var(--c-ink)]"> {t('Surveillance Completeness vs. Mean Immunisation Coverage (Aligned Axes)')} </h3>
          <p className="text-[12px] text-[var(--c-muted)]"> {t('Observing the synchronized collapse of health facility reporting rates alongside routine immunization delivery')} </p>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between text-[12px] font-semibold text-[var(--c-primary)] mb-1">
            <span>{t('A. National Surveillance Completeness Index (0.0 – 1.0)')}</span>
            <span className="font-mono text-[var(--c-danger)]">{t('Dropped 1.00 → 0.80 in 2021')}</span>
          </div>
        ...
        <div className="pt-2 border-t border-[var(--c-border)]">
          <div className="flex items-center justify-between text-[12px] font-semibold text-[var(--c-warning)] mb-1">
            <span>{t('B. Mean Routine Immunisation Coverage (%)')}</span>
            <span className="font-mono text-[var(--c-danger)]">{t('Halved: 92% → 45% in 2021')}</span>
          </div>
          <div className="h-[140px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={ANOMALY_SERIES} margin={{ top: 5, right: 20, left: -15, bottom: 0 }}>
      ...
      <div className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[8px] p-5 shadow-[var(--c-shadow-sm)]">
        <h3 className="text-[16px] font-semibold text-[var(--c-ink)] mb-1"> {t('Historical Epidemic Outbreak Years (Rolling-Median & MAD Detection)')} </h3>
        <p className="text-[12px] text-[var(--c-muted)] mb-4"> {t('Identified statistical anomalies where reported incidence exceeded 3× Median Absolute Deviation (MAD) over a 5-year moving window:')} </p>
          ...
          {OUTBREAK_YEARS.map((ob, idx) => (
            ...
            >
              <div>
                ...
                <div className="text-[11px] text-[var(--c-muted)] mt-0.5"> {t('Algorithm:')} {ob.method}
                </div>
              </div>
