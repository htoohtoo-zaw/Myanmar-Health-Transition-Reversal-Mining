    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[14px] text-[var(--c-muted)] mt-1 max-w-3xl">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto bg-[var(--c-surface)] px-3 py-1.5 rounded-[8px] border border-[var(--c-border-strong)] shadow-sm">
          ...
          <span className="text-[12px] font-semibold text-[var(--c-muted)]">Temporal Alignment:</span>
          <span className="text-[12px] font-mono font-bold text-[var(--c-primary)] bg-[var(--c-subtle-2)] px-2 py-0.5 rounded">
            Best Lag = 1 Year (t-1)
          </span>
        </div>
      </div>
      ...
      <div className="rounded-[8px] border border-[var(--c-warning-border)] bg-[var(--c-warning-bg)] p-5 shadow-[var(--c-shadow-sm)] border-l-4 border-l-[var(--c-warning)]">
        <div className="flex items-start gap-3.5">
          ...
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-[var(--c-warning)] mb-0.5">
              CRITICAL METHODOLOGICAL CAVEAT · CO-TRENDING CONFOUND
            </div>
            <h3 className="text-[16px] font-bold text-[var(--c-ink)] mb-1">
              Top Rule R1 (HepB_HIGH & MCV2_HIGH → Diphtheria_HIGH) is Non-Causal
            </h3>
            <p className="text-[13px] text-[var(--c-ink)] leading-relaxed">
            ...
            <div className="mt-2 text-[12px] text-[var(--c-muted)] font-medium bg-[var(--c-surface)]/60 p-2.5 rounded border border-[var(--c-warning-border)]">
          ...
          {topRules.map((rule) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded bg-[var(--c-subtle)] text-[var(--c-primary)]">
                  Rule {rule.id}
                </span>
                {rule.isCoTrendingConfound && (
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[var(--c-warning-bg)] text-[var(--c-warning)] border border-[var(--c-warning-border)]">
              ...
              <div className="grid grid-cols-3 gap-2 py-2 border-t border-b border-[var(--c-border)] text-center">
                <div>
                  <div className="text-[10px] text-[var(--c-muted)] uppercase font-medium">Support</div>
                  <div className="text-[13px] font-mono font-bold text-[var(--c-ink)]">
                    {(rule.support * 100).toFixed(0)}%
                  </div>
                </div>
                <div>
                  <div className="text-[10px] text-[var(--c-muted)] uppercase font-medium">Confidence</div>
                  <div className="text-[13px] font-mono font-bold text-[var(--c-ink)]">
                    {(rule.confidence * 100).toFixed(0)}%
                  </div>
                </div>
                <div>
                  <div className="text-[10px] text-[var(--c-muted)] uppercase font-medium">Lift</div>
